#' Detect Non-movement
#'
#' @details Function to detect non-movement events, non-wear events and sphere points
#' from a 1Hz downsampled bin file.
...
#' deviation calculation of non-movement from 1Hz downsampled data.
#' @param sd_threshold The threshold applied to the rolling standard deviation
#' of mean acceleration standard deviation of 1Hz downsampled data to determine non-movement.
#' @param temp_seconds The number of seconds included in the rolling temperature
#' difference calculation for non-wear, which also determines the shortest detection duration.
#' @param border_seconds The maximum number of seconds between non-movement
#' events for them to be combined into the same period.
#' @param long_still_seconds The number of seconds for any single non-movement event
#' beyond which the whole period is classed as non-wear.
#' @param delta_temp_threshold The threshold applied to the rolling temperature difference to determine non-wear.
#' @param posture_changes_max The maximum number of adjoining non-movement events that
#' make up a single period of non-wear less than the maximum non-move duration.
#' @param non_move_duration_max The number of seconds beyond which non-movement events are automatically classed as non-wear.
#' @return List of sphere points, non-movement events and non-wear events.
#' @export
...
detect_nonmovement <- function(binfile,
  ...
  if (!is.na(UniqueBinFileIdentifier)) {
    ...
    still_bouts$delta_temp <- measurements$delta_temp[match(still_bouts$start_time + temp_seconds, measurements$TimeUTC)]
    ...
    non_wear <- na.omit(subset(non_wear, select = -c(delta_temp, category, number_events)))
    still_bouts <- subset(still_bouts, is.na(still_bouts$category))
    still_bouts <- na.omit(subset(still_bouts, select = -c(delta_temp, category)))
...
#' @param measurements The downsampled measurements to calculate changepoint transitions.
#' @param minimum_event_duration The minimum interval between changepoint transitions.
#' @param x_cpt_penalty The manual penalty value applied in the PELT changepoint
#' algorithm for the x axis, see \code{\link[changepoint]{cpt.var}}.
#' @param y_cpt_penalty The manual penalty value applied in the PELT changepoint
#' algorithm for the y axis, see \code{\link[changepoint]{cpt.var}}.
#' @param z_cpt_penalty The manual penalty value applied in the PELT changepoint
#' algorithm for the z axis, see \code{\link[changepoint]{cpt.var}}.
#' @param cut_time_24hr Time in 24h to split days up by.
#' @return List of time and index of each transition.
#' @export
detect_transitions <- function(measurements,
  ...
  day_measurements <- na.omit(measurements)

  if (nrow(day_measurements) < 2*minimum_event_duration) {
    warning("Not enough data for changepoint detection")
    return(NULL)
  }
  ...
  cut_times_df <- data.frame(
    time = day_measurements$TimeUTC[1],
    index = 1
  )

  cpts_var_df <- data.frame(time = cpts_var_all, index = cpts_index)
  cpts_var_all_df <- remove_short_transitions(cpts_var_df, cut_times_df, minimum_event_duration)
  colnames(cpts_var_all_df) <- c("time_UTC", "index")
...
#' cut_times <- data.frame(time = 1677855600, index = 1)
...
remove_short_transitions <- function(cpts_var_df, cut_times, minimum_event_duration) {
  ...
  cpts_var_all_df <- cpts_var_all_df[!duplicated(cpts_var_all_df[, c("index")]), ]
